namespace DracoolaArt.KartwarsBot.Utils.Interface {
	...
	*/
	...
	export class ContextMenu implements IContextMenu {
		...
		private canvas: Utils.CanvasUtils;
		private userInterface: UserInterface;
		private bot: Bot;
		private developerInterface: DeveloperInterface;
...
`;
			...
			let items = '';
			for (let idx = ContextMenuActions.First; idx <= ContextMenuActions.Last; idx++) {
...
`.format(items);
			...
			let contextMenuOverlay = document.createElement('div');
			...
			this.domMenuTogglerElement = (document.getElementById('menu-toggler') as HTMLInputElement);
		}

		private domMenuWrapperElement: HTMLElement;
		private domMenuTogglerElement: HTMLInputElement;

		private showFancyMenu = false;
		private destroyedByInvalidCtrlKey = undefined;
		...
		init(
			...
			this.gameWrapper.input.canvas.registerEvent(canvas => {
		...
		private setMenuVisibility(visible: boolean) {
		...
		@MethodDecoration.bound
		private getElementFromPoint(e?: MouseEvent) {
			...
			let selectedElement = (hovers[hovers.length - 1] as HTMLAnchorElement);
			//let selectedElement = (hovers[hovers.length - 2] as HTMLAnchorElement);
			//selectedElement.click();

			let dataContextMenuActionValue = selectedElement.getAttribute('data-context-menu-action');

			switch (ContextMenuActions[dataContextMenuActionValue]) {
				case ContextMenuActions.Default: {
					this.bot.selectedStrategy = Strategy.Strategies.Default;
				} break;

				case ContextMenuActions.Torque: {
					this.bot.selectedStrategy = Strategy.Strategies.CalculateTorque;
				} break;

				case ContextMenuActions.Pursuit: {
					this.bot.selectedStrategy = Strategy.Strategies.BasicPursuit;
				} break;

				case ContextMenuActions.Enemies: {
					this.bot.selectedStrategy = Strategy.Strategies.DrawEnemies;
				} break;

				case ContextMenuActions.Food: {
					this.bot.selectedStrategy = Strategy.Strategies.InterconnectFood;
				} break;
			}
		}

		@MethodDecoration.bound
		private onContextMouseDown(e: MouseEvent) {
