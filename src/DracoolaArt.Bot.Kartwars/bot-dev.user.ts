...
// @namespace    http://scripts.local.com/
// @version      1.0.475
...
// @updateURL    http://scripts.local.com/bot-dev.user.js
// @downloadURL  http://scripts.local.com/bot-dev.user.js
...
!function (window, document) {
	let baseURL = 'http://scripts.local.com/';
	let baseScriptPath = `${baseURL}lib/`;
	let baseStylePath = `${baseURL}style/`;

	window.botSettings = {
		LOAD_DEBUG_SCRIPTS: true,
		LOAD_GIT_SCRIPTS: false,
	};

	window.GM_info = GM_info;

	function initLoader(baseURL: string, baseScriptPath: string) {
	...
	function loadStylesheet(fileURL: string) {
		fileURL += `?time=${(+new Date())}`;

		let remoteLink = document.createElement('link');
	...
	if (window.botSettings.LOAD_GIT_SCRIPTS) {
		$.ajax({
			...
			let baseURL = `https://cdn.rawgit.com/kmataru/kartwars.io-bot/${sha}/src/DracoolaArt.Bot.Kartwars/`;
			let baseScriptPath = `${baseURL}lib/`;
			let baseStylePath = `${baseURL}style/`;
